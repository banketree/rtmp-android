...
public class LibVLC {
	...
	public void init(Context context) throws LibVlcException { // 初始化
		...
		if (!mIsInitialized) {
			...
			mCachePath = TFilePathManager.getInstance().getCachePath();
			nativeInit();
	...
	// 截图
	public native boolean takeSnapShot(int num, String filePath, int width,
			int height);

	public boolean takeSnapShot(String filePath, int width, int height) {
		return takeSnapShot(0, filePath, width, height);
	}
