...
public class VideoPlayerActivity extends Activity implements IVideoPlayer { // 视频
	...
	private MediaRouter.SimpleCallback mMediaRouterCallback;// 监听可用媒体路由的事件
	...
	private SharedPreferences mSettings; // 配置文件

	/** Overlay */
	private View mOverlayHeader; // 头
	private View mOverlayOption; //
	private View mOverlayProgress; // 进度
	private View mOverlayBackground; // 重叠
	...
	private boolean mDragging; // 是否拖动
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		createPresentation();
		setContentView(mPresentation == null ? R.layout.player
				: R.layout.player_remote_control);
		...
		if (mPresentation != null
				&& !mSettings
						.getBoolean(
								"enable_secondary_display_hardware_acceleration",
								false)) {
	...
	protected void onPause() {
		...
		try {
			String strPath = Environment.getExternalStorageDirectory()
					.getAbsolutePath();
			// mLibVLC.takeSnapShot(strPath, 160, 160);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Add or remove MediaRouter callbacks. This is provided for version
	 * targeting.
	 * 
	 * @param add
	...
	private void mediaRouterAddCallback(boolean add) {
		if (!LibVlcUtil.isJellyBeanMR1OrLater() || mMediaRouter == null)
			return;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (coef > 2 && mPresentation == null) {
				...
				if (VLCManager.getInstance().hasNavBar())
					showOverlay();
			}
	...
	private void doSeekTouch(float coef, float gesturesize, boolean seek) {
		...
		if (length > 0)
			...
			showInfo(String.format("%s%s (%s)", jump >= 0 ? "+" : "",
					TStringUtils.millisToString(jump),
					TStringUtils.millisToString(time + jump)), 1000);
		else
	...
	private final OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
		...
		public void onProgressChanged(SeekBar seekBar, int progress,
			...
			if (fromUser && mCanSeek) {
				...
				setOverlayProgress();
				mTime.setText(TStringUtils.millisToString(progress));
    ...
    */
	...
	private void hideOverlay(boolean fromUser) {
		if (mShowing) {
			...
			if (mPresentation != null) {
				mOverlayBackground.startAnimation(AnimationUtils.loadAnimation(
						this, android.R.anim.fade_out));
	...
	private void updateOverlayPausePlay() {
		...
		// mPlayPause
		// .setBackgroundResource(mLibVLC.isPlaying() ?
		// R.drawable.ic_pause_circle_big_o
	...
	private int setOverlayProgress() {
		...
		if (time >= 0)
			mTime.setText(TStringUtils.millisToString(time));
		if (length >= 0)
	...
	private void removePresentation() {
		...
		finish(); // TODO restore the video on the new display instead of
					// closing
		if (mPresentation != null)
			mPresentation.dismiss();
		mPresentation = null;
	...
	private final class SecondaryDisplay extends Presentation {
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			mSubtitlesSurfaceHolder.setFormat(PixelFormat.RGBA_8888);
			mSubtitlesSurface.setZOrderMediaOverlay(true);
			mSubtitlesSurfaceHolder
					.addCallback(activity.mSubtitlesSurfaceCallback);
			 ...
			 */
			if (mLibVLC != null
					&& mLibVLC.getHardwareAcceleration() == LibVLC.HW_ACCELERATION_FULL)
				mSubtitlesSurface.setVisibility(View.VISIBLE);
