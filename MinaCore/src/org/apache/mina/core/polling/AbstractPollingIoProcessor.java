...
/**
	...
	private int handleNewSessions() {
		...
		for (S session = newSessions.poll(); session != null; session = newSessions
				.poll()) {
			if (addNow(session)) {
	...
	private int removeSessions() {
		...
		for (S session = removingSessions.poll(); session != null; session = removingSessions
				.poll()) {
	...
	private int writeFile(S session, WriteRequest req,
		...
		if (region.getRemainingBytes() > 0) {
			...
			if (hasFragmentation) {
				...
				length = (int) Math.min(Integer.MAX_VALUE,
						region.getRemainingBytes());
			}
	...
	public void updateTrafficControl(S session) {
		...
		try {
			setInterestedInWrite(session, !session.getWriteRequestQueue()
					.isEmpty(session) && !session.isWriteSuspended());
		} catch (Exception e) {
			IoFilterChain filterChain = session.getFilterChain();
			filterChain.fireExceptionCaught(e);
		}
