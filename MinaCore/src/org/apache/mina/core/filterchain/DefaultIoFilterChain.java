...
public class DefaultIoFilterChain implements IoFilterChain {
	...
	private final static Logger LOGGER = LoggerFactory
			.getLogger(DefaultIoFilterChain.class);
	...
	private class HeadFilter extends IoFilterAdapter {
		...
		public void filterClose(NextFilter nextFilter, IoSession session)
				throws Exception {
			((AbstractIoSession) session).getProcessor().remove(
	...
	private static class TailFilter extends IoFilterAdapter {
		...
		public void sessionCreated(NextFilter nextFilter, IoSession session)
				throws Exception { //会话创建
			try {
				session.getHandler().sessionCreated(session); //(与服务器建立关系)
			} finally {
				// Notify the related future.
				ConnectFuture future = (ConnectFuture) session
						.removeAttribute(SESSION_CREATED_FUTURE);
	...
	private class EntryImpl implements Entry { // 实体接口
