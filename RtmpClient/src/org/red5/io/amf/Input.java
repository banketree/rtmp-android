...
public class Input extends BaseInput implements org.red5.io.object.Input {
	 ...
	 */
	private final static String bufferToString(
			final java.nio.ByteBuffer strBuf, int len) {
		// Trac #601 - part of the problem seems to be a null byte buffer
		String string = null;
		if (strBuf != null) {
			int pos = strBuf.position();
			// log.debug("String buf - position: {} limit: {}", pos, (pos +
			// len));
	...
	public Object readArray(Deserializer deserializer, Type target) {
		// log.debug("readArray - deserializer: {} target: {}", deserializer,
		// target);
		Object result = null;
		...
		// To maintain conformance to the Input API, we should convert the
		// output
		...
		if (collection.isArray()) {
			result = ArrayUtils.toArray(collection.getComponentType(),
					resultCollection);
		} else {
	...
	public Object readMap(Deserializer deserializer, Type target) {
		...
		Object result;
		final Map<Object, Object> mixedResult = new LinkedHashMap<Object, Object>(
				maxNumber);
