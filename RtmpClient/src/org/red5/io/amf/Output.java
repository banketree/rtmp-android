...
public class Output extends BaseOutput implements org.red5.io.object.Output {
	...
	// try {
	// cacheManager = new CacheManager(System.getProperty("red5.root") +
	// File.separator + "conf"
	...
	public void writeObject(Object object, Serializer serializer) {
		/*
		 * if (checkWriteReference(object)) { return; } storeReference(object);
		 * // Create new map out of bean properties BeanMap beanMap = new
		 * BeanMap(object); // Set of bean attributes Set set =
		 * beanMap.keySet(); if ((set.size() == 0) || (set.size() == 1 &&
		 * beanMap.containsKey("class"))) { // BeanMap is empty or can only
		 * access "class" attribute, skip it writeArbitraryObject(object,
		 * serializer); return; }
		 * 
		 * // Write out either start of object marker for class name or "empty"
		 * start of object marker Class<?> objectClass = object.getClass(); if
		 * (!objectClass.isAnnotationPresent(Anonymous.class)) {
		 * buf.put(AMF.TYPE_CLASS_OBJECT); putString(buf,
		 * serializer.getClassName(objectClass)); } else {
		 * buf.put(AMF.TYPE_OBJECT); }
		 * 
		 * if (object instanceof ICustomSerializable) { ((ICustomSerializable)
		 * object).serialize(this, serializer); buf.put((byte) 0x00);
		 * buf.put((byte) 0x00); buf.put(AMF.TYPE_END_OF_OBJECT); return; }
		 * 
		 * // Iterate thru entries and write out property names with separators
		 * for (Object key : set) { String fieldName = key.toString();
		 * log.debug("Field name: {} class: {}", fieldName, objectClass);
		 * 
		 * Field field = getField(objectClass, fieldName); Method getter =
		 * getGetter(objectClass, beanMap, fieldName);
		 * 
		 * // Check if the Field corresponding to the getter/setter pair is
		 * transient if (!serializeField(serializer, objectClass, fieldName,
		 * field, getter)) { continue; }
		 * 
		 * putString(buf, fieldName); serializer.serialize(this, field, getter,
		 * object, beanMap.get(key)); } // Write out end of object mark
	...
	protected boolean serializeField(Serializer serializer,
		...
		// to prevent, NullPointerExceptions, get the element first and check if
		// it's null.
		// Element element = getSerializeCache().get(objectClass);
		// Map<String, Boolean> serializeMap = (element == null ? null :
		// (Map<String, Boolean>) element.getObjectValue());
	...
	protected Field getField(Class<?> objectClass, String keyName) {
		...
		// Map<String, Field> fieldMap = (element == null ? null : (Map<String,
		// Field>) element.getObjectValue());
		...
		if (fieldMap.containsKey(keyName)) {
			...
			for (Class<?> clazz = objectClass; !clazz.equals(Object.class); clazz = clazz
					.getSuperclass()) {
	...
	// @SuppressWarnings("unchecked")
	// protected Method getGetter(Class<?> objectClass, BeanMap beanMap, String
	// keyName) {
	// check element to prevent null pointer
	// Element element = getGetterCache().get(objectClass);
	// Map<String, Method> getterMap = (element == null ? null : (Map<String,
	// Method>) element.getObjectValue());
	// if (getterMap == null) {
	...
	protected static byte[] encodeString(String string) {
		...
		// byte[] encoded = (element == null ? null : (byte[])
	...
	// if (stringCache == null) {
	// stringCache =
	// getCacheManager().getCache("org.red5.io.amf.Output.stringCache");
	// }
	//
	// return stringCache;
	// }
	...
	// serializeCache =
	// getCacheManager().getCache("org.red5.io.amf.Output.serializeCache");
	// }
	//
	...
	// if (fieldCache == null) {
	// fieldCache =
	// getCacheManager().getCache("org.red5.io.amf.Output.fieldCache");
	// }
	//
	// return fieldCache;
	// }
	...
	// if (getterCache == null) {
	// getterCache =
	// getCacheManager().getCache("org.red5.io.amf.Output.getterCache");
	// }
	//
	// return getterCache;
	// }
