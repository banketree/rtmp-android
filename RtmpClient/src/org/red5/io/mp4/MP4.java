...
public class MP4 implements IMP4 {
	...
	public MP4(File file) {
		this.file = file;
		/*
		 * try { MP4Reader reader = new MP4Reader(this.file); ITag tag =
		 * reader.createFileMeta(); if (metaService == null) { metaService = new
		 * MetaService(this.file); } metaData =
		 * metaService.readMetaData(tag.getBody()); reader.close(); } catch
		 * (Exception e) { log.error("An error occurred looking for metadata:",
		 * e); }
		 */
	}
