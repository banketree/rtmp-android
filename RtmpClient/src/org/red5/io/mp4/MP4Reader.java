 ...
 * 
 * NOTE: This class is not implemented as thread-safe, the caller should ensure
 * the thread-safety.
 * <p>
 ...
 * components:
 * <ul>
 * <li>NetStream.Play.FileStructureInvalid: This event is sent if the player
 * detects an MP4 with an invalid file structure. Flash Player cannot play files
 * that have invalid file structures.</li>
 * <li>NetStream.Play.NoSupportedTrackFound: This event is sent if the player
 * does not detect any supported tracks. If there aren't any supported video,
 * audio or data tracks found, Flash Player does not play the file.</li>
 * </ul>
 * </p>
 * 
...
public class MP4Reader implements IoConstants, ITagReader,
	...
	/** Audio packet prefix */
	public final static byte[] PREFIX_AUDIO_FRAME = new byte[] { (byte) 0xaf,
			(byte) 0x01 };

	/** Audio config aac main */
	public final static byte[] AUDIO_CONFIG_FRAME_AAC_MAIN = new byte[] {
			(byte) 0x0a, (byte) 0x10 };

	/** Audio config aac lc */
	public final static byte[] AUDIO_CONFIG_FRAME_AAC_LC = new byte[] {
			(byte) 0x12, (byte) 0x10 };

	/** Audio config sbr */
	public final static byte[] AUDIO_CONFIG_FRAME_SBR = new byte[] {
			(byte) 0x13, (byte) 0x90, (byte) 0x56, (byte) 0xe5, (byte) 0xa5,
			(byte) 0x48, (byte) 0x00 };

	/** Video packet prefix for the decoder frame */
	public final static byte[] PREFIX_VIDEO_CONFIG_FRAME = new byte[] {
			(byte) 0x17, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00 };

	/** Video packet prefix for key frames */
	public final static byte[] PREFIX_VIDEO_KEYFRAME = new byte[] {
			(byte) 0x17, (byte) 0x01 };

	/** Video packet prefix for standard frames (interframe) */
	public final static byte[] PREFIX_VIDEO_FRAME = new byte[] { (byte) 0x27,
			(byte) 0x01 };
	...
	/**
	 * Container for metadata and any other tags that should be sent prior to
	 * media data.
	 */
	private LinkedList<ITag> firstTags = new LinkedList<ITag>();

	/**
	 * Container for seek points in the video. These are the time stamps for the
	 * key frames.
	 */
	...
	/**
	 * Creates MP4 reader from file input stream, sets up metadata generation
	 * flag.
	 * 
	 * @param f
	 *            File input stream
	...
	public void decodeHeader() {
		try {
			...
			// log.debug("Atom int types - free={} wide={}",
			// MP4Atom.typeToInt("free"), MP4Atom.typeToInt("wide"));
			// keep a running count of the number of atoms found at the "top"
			// levels
			int topAtoms = 0;
			// we want a moov and an mdat, anything else throw the invalid file
			// type error
			while (topAtoms < 2) {
				...
				case 1836019574: // moov
					...
					if (mvhd != null) {
						...
						log.debug("Time scale {} Duration {}", timeScale,
								duration);
					}

					/*
					 * nothing needed here yet MP4Atom meta =
					 * moov.lookup(MP4Atom.typeToInt("meta"), 0); if (meta !=
					 * null) { log.debug("Meta atom found"); log.debug("{}",
					 * ToStringBuilder.reflectionToString(meta)); }
					 */

					// we would like to have two tracks, but it shouldn't be a
					// requirement
					int loops = 0;
					int tracks = 0;
					do {

						MP4Atom trak = moov.lookup(MP4Atom.typeToInt("trak"),
								loops);
						if (trak != null) {
							log.debug("Track atom found");
							log.debug("trak children: {}", trak.getChildren());
							// trak: tkhd, edts, mdia
							MP4Atom tkhd = trak.lookup(
									MP4Atom.typeToInt("tkhd"), 0);
							if (tkhd != null) {
								log.debug("Track header atom found");
								log.debug("tkhd children: {}",
										tkhd.getChildren());
								if (tkhd.getWidth() > 0) {
									width = tkhd.getWidth();
									height = tkhd.getHeight();
									log.debug("Width {} x Height {}", width,
											height);
								}
							}

							MP4Atom edts = trak.lookup(
									MP4Atom.typeToInt("edts"), 0);
							if (edts != null) {
								log.debug("Edit atom found");
								log.debug("edts children: {}",
										edts.getChildren());
								// log.debug("Width {} x Height {}",
								// edts.getWidth(), edts.getHeight());
							}

							MP4Atom mdia = trak.lookup(
									MP4Atom.typeToInt("mdia"), 0);
							if (mdia != null) {
								...
								MP4Atom minf = mdia.lookup(
										MP4Atom.typeToInt("minf"), 0);
								if (minf != null) {
									log.debug("Media info atom found");
									// minf: (audio) smhd, dinf, stbl / (video)
									// vmhd,
									// dinf, stbl

									MP4Atom smhd = minf.lookup(
											MP4Atom.typeToInt("smhd"), 0);
									if (smhd != null) {
										log.debug("Sound header atom found");
										MP4Atom dinf = minf.lookup(
												MP4Atom.typeToInt("dinf"), 0);
										if (dinf != null) {
											...
											log.debug(
													"Sound dinf children: {}",
													dinf.getChildren());
											MP4Atom dref = dinf.lookup(
													MP4Atom.typeToInt("dref"),
													0);
											if (dref != null) {
										...
										MP4Atom stbl = minf.lookup(
												MP4Atom.typeToInt("stbl"), 0);
										if (stbl != null) {
											log.debug("Sample table atom found");
											// stbl: stsd, stts, stss, stsc,
											// stsz, stco,
											// stsh
											log.debug(
													"Sound stbl children: {}",
											...
											MP4Atom stsd = stbl.lookup(
													MP4Atom.typeToInt("stsd"),
													0);
											if (stsd != null) {
												// stsd: mp4a
												...
												MP4Atom mp4a = stsd
														.getChildren().get(0);
												// could set the audio codec
												// here
												setAudioCodecId(MP4Atom
														.intToType(mp4a
																.getType()));
												// log.debug("{}",
												// ToStringBuilder.reflectionToString(mp4a));
												log.debug("Sample size: {}",
														mp4a.getSampleSize());
												...
												if (mp4a.getChildren().size() > 0) {
													...
													log.debug(
															"{}",
															ToStringBuilder
																	.reflectionToString(esds));
													MP4Descriptor descriptor = esds
															.getEsd_descriptor();
													log.debug(
															"{}",
															ToStringBuilder
													...
													if (descriptor != null) {
														Vector<MP4Descriptor> children = descriptor
																.getChildren();
														for (int e = 0; e < children
																.size(); e++) {
															MP4Descriptor descr = children
																	.get(e);
															log.debug(
																	"{}",
																	ToStringBuilder
															...
															if (descr
																...
																for (int e2 = 0; e2 < children2
																	...
																	if (descr2
																			.getType() == MP4Descriptor.MP4DecSpecificInfoDescriptorTag) {
																		// we
																		// only
																		// want
																		// the
																		// MP4DecSpecificInfoDescriptorTag
																		audioDecoderBytes = descr2
																				.getDSID();
																		// compare
																		// the
																		// bytes
											...
											// stsc - has Records
											MP4Atom stsc = stbl.lookup(
													MP4Atom.typeToInt("stsc"),
													0);
											if (stsc != null) {
												log.debug("Sample to chunk atom found");
												audioSamplesToChunks = stsc
														.getRecords();
												log.debug("Record count: {}",
														audioSamplesToChunks
																.size());
												MP4Atom.Record rec = audioSamplesToChunks
														.firstElement();
												log.debug(
														"Record data: Description index={} Samples per chunk={}",
														rec.getSampleDescriptionIndex(),
														rec.getSamplesPerChunk());
											}
											// stsz - has Samples
											MP4Atom stsz = stbl.lookup(
													MP4Atom.typeToInt("stsz"),
													0);
											if (stsz != null) {
												...
												log.debug("Sample size: {}",
														stsz.getSampleSize());
												log.debug("Sample count: {}",
														audioSamples.size());
											}
											// stco - has Chunks
											MP4Atom stco = stbl.lookup(
													MP4Atom.typeToInt("stco"),
													0);
											if (stco != null) {
												log.debug("Chunk offset atom found");
												// vector full of integers
												audioChunkOffsets = stco
														.getChunks();
											...
											MP4Atom stts = stbl.lookup(
													MP4Atom.typeToInt("stts"),
													0);
											if (stts != null) {
												...
												log.debug("Record count: {}",
														records.size());
												MP4Atom.TimeSampleRecord rec = records
														.firstElement();
												log.debug(
														"Record data: Consecutive samples={} Duration={}",
														rec.getConsecutiveSamples(),
														rec.getSampleDuration());
												// if we have 1 record then all
												...
												audioSampleDuration = rec
														.getSampleDuration();
											}
										}
									}
									MP4Atom vmhd = minf.lookup(
											MP4Atom.typeToInt("vmhd"), 0);
									if (vmhd != null) {
										...
										if (stbl != null) {
											...
											// stsz - sample size
											// stco - chunk offset
											// ctts - (composition) time to
											// sample
											// stss - sync sample
											// sdtp - independent and disposable
											// samples
											...
											if (stsd != null) {
												log.debug("Sample description atom found");
												log.debug(
														"Sample description (video) stsd children: {}",
														stsd.getChildren());
												MP4Atom avc1 = stsd
														.lookup(MP4Atom
																.typeToInt("avc1"),
												...
												if (avc1 != null) {
													log.debug(
															"AVC1 children: {}",
															avc1.getChildren());
													// set the video codec here
													// - may be avc1 or mp4v
													setVideoCodecId(MP4Atom
															.intToType(avc1
																	.getType()));
													// video decoder config
													// TODO may need to be
													// generic later
													MP4Atom codecChild = avc1
															.lookup(MP4Atom
																	.typeToInt("avcC"),
																	0);
													if (codecChild != null) {
											...
											if (stsc != null) {
												log.debug("Sample to chunk atom found");
												videoSamplesToChunks = stsc
														.getRecords();
												log.debug("Record count: {}",
														videoSamplesToChunks
																.size());
												MP4Atom.Record rec = videoSamplesToChunks
														.firstElement();
												log.debug(
														"Record data: Description index={} Samples per chunk={}",
														rec.getSampleDescriptionIndex(),
											...
											if (stsz != null) {
												...
												videoSamples = stsz
														.getSamples();
												// if sample size is 0 then the
												// table must be checked due
											...
											if (stco != null) {
												log.debug("Chunk offset atom found");
												// vector full of integers
												videoChunkOffsets = stco
														.getChunks();
												log.debug("Chunk count: {}",
														videoChunkOffsets
																.size());
											}
											// stss - has Sync - no sync means
											// all samples are keyframes
											MP4Atom stss = stbl.lookup(
													MP4Atom.typeToInt("stss"),
													0);
											if (stss != null) {
											...
											if (stts != null) {
												...
												videoSampleDuration = rec
														.getSampleDuration();
											}
											// ctts - (composition) time to
											// sample
											MP4Atom ctts = stbl.lookup(
													MP4Atom.typeToInt("ctts"),
													0);
											if (ctts != null) {
												log.debug("Composition time to sample atom found");
												// vector full of integers
												compositionTimes = ctts
														.getCompositionTimeToSamplesRecords();
												log.debug("Record count: {}",
														compositionTimes.size());
												if (log.isTraceEnabled()) {
													for (MP4Atom.CompositionTimeSampleRecord rec : compositionTimes) {
														...
														log.trace(
																"Record data: Consecutive samples={} Offset={}",
																rec.getConsecutiveSamples(),
																rec.getSampleOffset());
													}
												}
					...
					log.trace(
							"Busted out of track loop with {} tracks after {} loops",
							tracks, loops);
					// calculate FPS
					fps = (videoSampleCount * timeScale) / (double) duration;
					log.debug("FPS calc: ({} * {}) / {}", new Object[] {
				...
				case 2003395685: // wide
					break;
				default:
					log.warn("Unexpected atom: {}",
							MP4Atom.intToType(atom.getType()));
				}
			}
	...
	ITag createFileMeta() {
		...
		if (hasAudio) {
			...
			if (audioSamples != null) {
				audioMap.put("length_property", audioSampleDuration
						* audioSamples.size());
		...
		if (hasVideo) {
			...
			if (videoSamples != null) {
				videoMap.put("length_property", videoSampleDuration
						* videoSamples.size());
				// release some memory, since we're done with the vectors
				videoSamples.clear();
				videoSamples = null;
	...
	/**
	 * Tag sequence MetaData, Video config, Audio config, remaining audio and
	 * video
	 * 
	 * Packet prefixes: 17 00 00 00 00 = Video extra data (first video packet)
	 * 17 01 00 00 00 = Video keyframe 27 01 00 00 00 = Video interframe af 00
	 * ... 06 = Audio extra data (first audio packet) af 01 = Audio frame
	 * 
	 * Audio extra data(s): af 00 = Prefix 11 90 4f 14 = AAC Main = aottype 0 12
	 * 10 = AAC LC = aottype 1 13 90 56 e5 a5 48 00 = HE-AAC SBR = aottype 2 06
	 * = Suffix
	 * 
	 * Still not absolutely certain about this order or the bytes - need to
	 * verify later
	 */
	private void createPreStreamingTags(int timestamp, boolean clear) {
		...
		if (hasVideo) {
			...
			if (videoDecoderBytes != null) {
				...
				// log.debug("Video decoder bytes: {}",
				// HexDump.byteArrayToHexString(videoDecoderBytes));
				// try {
				// log.debug("Video bytes data: {}", new
				// String(videoDecoderBytes, "UTF-8"));
			...
			tag = new Tag(IoConstants.TYPE_VIDEO, timestamp, body.position(),
					null, 0);
			body.flip();
			tag.setBody(body);
			// add tag
		...
		if (hasAudio) {
			...
			if (audioDecoderBytes != null) {
				// because of other processing we do this check
				// if (log.isDebugEnabled()) {
				// log.debug("Audio decoder bytes: {}",
				// HexDump.byteArrayToHexString(audioDecoderBytes));
				// try {
				// log.debug("Audio bytes data: {}", new
				// String(audioDecoderBytes, "UTF-8"));
				// } catch (UnsupportedEncodingException e) {
				// log.error("", e);
				// }
				// }
				...
				// default to aac-lc when the esds doesnt contain descripter
				// bytes
				body.put(AUDIO_CONFIG_FRAME_AAC_LC);
			}
			body.put((byte) 0x06); // suffix
			tag = new Tag(IoConstants.TYPE_AUDIO, timestamp, body.position(),
					null, tag.getBodySize());
			body.flip();
			tag.setBody(body);
			// add tag
			firstTags.add(tag);
	...
	public synchronized ITag readTag() {
		...
		// log.debug("Read tag - dst: {} base: {} time: {}", new
		// Object[]{frameTs, baseTs, time});
		...
		// log.debug("Read tag - type: {} body size: {}", (type == TYPE_AUDIO ?
		// "Audio" : "Video"), tag.getBodySize());
	...
	/**
	 * Performs frame analysis and generates metadata for use in seeking. All
	 * the frames are analyzed and sorted together based on time and offset.
	 */
			...
			for (int i = 0; i < videoSamplesToChunks.size(); i++) {
				...
				for (int chunk = firstChunk; chunk <= lastChunk; chunk++) {
					...
					while (sampleCount > 0) {
						...
						// calculate ts
						double ts = (videoSampleDuration * (sample - 1))
								/ videoTimeScale;
						...
						if (compositeTimeEntry != null) {
							...
							int consecutiveSamples = compositeTimeEntry
									.getConsecutiveSamples();
							frame.setTimeOffset(compositeTimeEntry
									.getSampleOffset());
							// increment our count
							compositeIndex++;
							if (compositeIndex - consecutiveSamples == 0) {
								...
								if (!compositionTimes.isEmpty()) {
									// get the next one
									compositeTimeEntry = compositionTimes
											.remove(0);
								}
								// reset
								compositeIndex = 0;
		...
		if (audioSamplesToChunks != null) {
			...
			for (int i = 0; i < audioSamplesToChunks.size(); i++) {
				...
				for (int chunk = firstChunk; chunk <= lastChunk; chunk++) {
					...
					while (sampleCount > 0) {
						// calculate ts
						double ts = (audioSampleDuration * (sample - 1))
								/ audioTimeScale;
						// sample size
	...
	/**
	 * Put the current position to pos. The caller must ensure the pos is a
	 * valid one.
	 * 
	 * @param pos
	...
	public void position(long pos) {
		...
		for (int f = 0; f < len; f++) {
			...
			if (pos == offset || (offset > pos && frame.isKeyFrame())) {
				...
				if (!frame.isKeyFrame()) {
					log.debug(
							"Frame #{} was not a key frame, so trying again..",
							f);
					continue;
				}
