 ...
 */
public class ServiceInvoker implements IServiceInvoker {// 使远程调用，调用服务。
	 ...
	 */
	// private Set<IServiceResolver> serviceResolvers = new
	// HashSet<IServiceResolver>();
	...
	public boolean invoke(IServiceCall call, Object service) {
		...
		log.debug("Service: {} method name: {}", call.getServiceName(),
				methodName);
		// pull off the prefixes since java doesnt allow this on a method name
		if (methodName.charAt(0) == '@') {
			log.debug(
					"Method name contained an illegal prefix, it will be removed: {}",
					methodName);
		...
		try {
			...
			final DeclareProtected annotation = method
					.getAnnotation(DeclareProtected.class);
			if (annotation != null) {
				if (!conn.getClient().hasPermission(conn,
						annotation.permission())) {
					// Client doesn't have required permission
					log.debug(
							"Client {} doesn't have required permission {} to call {}",
							new Object[] { conn.getClient(),
									annotation.permission(), method });
					throw new NotAllowedException(
							"you are not allowed to execute this method");
				}
			}
