...
/**
 * Basic service call (remote call) implementation 基本的服务调用（远程调用）的实现
 * 
...
public class Call implements IServiceCall, Externalizable {
	 ...
	 * NOTE that it is possible that this error is returned in the situation
	 * where the method has been invoked on the server the connection has failed
	 * before the result returned could be read. There is no way to establish
	...
	/**
	 * Creates call from given service name, method name and array of call
	 * parameters
	 * 
	 * @param name
	 *            Service name
