...
public class ConnectionConsumer implements IPushableConsumer,
		IPipeConnectionListener { // rtmp连接者
	...
	/**
	 * Whether or not the chunk size has been sent. This seems to be required
	 * for h264.
	 */
	...
	public ConnectionConsumer(RTMPConnection conn, int videoChannel,
		...
		log.debug("Channel ids - video: {} audio: {} data: {}", new Object[] {
				videoChannel, audioChannel, dataChannel });
		this.conn = conn;
	...
	public void pushMessage(IPipe pipe, IMessage message) {
		...
		if (message instanceof ResetMessage) {
			...
			// log.debug("Message timestamp: {} durtion:{}", eventTime,
			// eventTime - lastts);
			lastts = eventTime;
			if (eventTime < 0) {
	...
	public void onOOBControlMessage(IMessageComponent source, IPipe pipe,
		...
		if ("ConnectionConsumer".equals(oobCtrlMsg.getTarget())) {
			...
			if ("pendingCount".equals(serviceName)) {
				...
				// conn.getStreamByChannelId(video.getId());
				// if (stream != null) {
				...
				// Return the current delta between sent bytes and bytes the
				// client
				// reported to have received, and the interval the client should
				// use
