...
/**
 * Abstract base implementation of IStream. Contains codec information, stream
 * name, scope, event handling meand, provides stream start and stop operations.
 * 
 * @see org.red5.server.stream.IStream
 */
public abstract class AbstractStream implements IStream { // 抽象流

	/**
	 * Current state
	 */
	protected StreamState state = StreamState.UNINIT; // 当前状态

	/**
	 * Stream name
	 */
	private String name; // 流名字

	/**
	 * Stream audio and video codec information
	 */
	private IStreamCodecInfo codecInfo; // 编码信息

	/**
	 * Stores the streams metadata
	 */
	protected Notify metaData; // 流数据
	 ...
	 */
	public IStreamCodecInfo getCodecInfo() { // 获取编码信息
		return codecInfo;
	}
