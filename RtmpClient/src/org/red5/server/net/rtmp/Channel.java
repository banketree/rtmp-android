...
/**
 * Identified connection that transfers packets. 认证连接数据包
 */
public class Channel {
	 ...
	 *            Event data
	 */
	public void write(IRTMPEvent event) { // 写入数据
	 ...
	 *            Status
	 */
	public void sendStatus(Status status) { //发送状态通知
		...
		if (andReturn) {
			...
			if (status.getCode().equals(StatusCodes.NS_PLAY_START)) {
				...
				// if
				// (scope.getContext().getApplicationContext().containsBean(IRtmpSampleAccess.BEAN_NAME))
				// {
				// IRtmpSampleAccess sampleAccess = (IRtmpSampleAccess)
				...
				// notify.setData(IoBuffer.wrap(new byte[] { 0x01, (byte)
				// (audioAccess ? 0x01 : 0x00), 0x01, (byte) (videoAccess ? 0x01
				// : 0x00) }));
