 ...
 */
public class RTMPMinaConnection extends RTMPConnection {// RTMP mina连接
	 ...
	 */
	private volatile IoSession ioSession; // 会话操作
	...
	public void close() { // 关闭连接
		...
		if (ioSession != null) {
			...
			// only close socket after all pending data has been sent, this does
			// not
	...
	public boolean connect(Object[] params) { // 连接
		...
		if (client != null) {
			try {
				...
				// RTMPMinaConnectionMXBean.class, oName);
			} catch (Exception e) {
		...
		if (ioSession != null) {
			...
			// TrafficShapingFilter filter = (TrafficShapingFilter)
			// ctx.getBean("bandwidthFilter");
			// //load the each after the last
			// ioSession.getFilterChain().addAfter("protocolFilter",
			// "bandwidthFilter", filter);
			// //notify client about new bandwidth settings (in bytes per
			// second)
	 ...
	 */
	public IoSession getIoSession() { // 返回mina 操作会话
		return ioSession;
	}

	/** {@inheritDoc} */
	@Override
	public long getPendingMessages() { // 获取意图消息
		if (ioSession != null) {
	...
	public void invokeMethod(String method) { // 调用方法
		invoke(method);
	}
	...
	/** {@inheritDoc} */
	@Override
	public void write(Packet out) { // 写入
		if (ioSession != null) {
			writingMessage(out);
			ioSession.write(out);
		}
