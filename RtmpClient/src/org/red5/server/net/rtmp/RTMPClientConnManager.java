...
public class RTMPClientConnManager implements IRTMPConnManager { // rtmp客户端连接管理

	private static final Logger log = LoggerFactory
			.getLogger(RTMPClientConnManager.class);
	...
	public static RTMPClientConnManager getInstance() { // 获取实例
		return instance;
	}
	 ...
	 * @return connection
	 */
	public RTMPConnection getConnection(int clientId) { // 获取连接
	...
	/**
	 * Removes a connection matching the client id specified. If found, the
	 * connection will be returned.
	 * 
	 * @param clientId
	 * @return connection
	 */
	public RTMPConnection removeConnection(int clientId) { // 移除连接
		RTMPConnection connReturn = null;
		for (RTMPConnection conn : rtmpConnections) {
			if (conn.getId() == clientId) {
				connReturn = conn;
	 ...
	 */
	public Collection<RTMPConnection> removeConnections() { // 移除所有连接
		rtmpConnections.clear();
		return null;
	}

	public RTMPConnection createConnection(Class<?> connCls) { // 生成连接
