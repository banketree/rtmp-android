 ...
 * 事件聚合数据
 */
public class Aggregate extends BaseEvent implements IoConstants,
		IStreamData<Aggregate>, IStreamPacket {
	...
	/**
	 * Breaks-up the aggregate into its individual parts and returns them as a
	 * list. The parts are returned based on the ordering of the aggregate
	 * itself.
	 * 
	...
	public LinkedList<IRTMPEvent> getParts() {
		...
		do {
			...
			/*
			 * timestamp = ntohap((GETIBPOINTER(buffer) + 4)); 0x12345678 == 34
			 * 56 78 12
			 */
			...
			// the back pointer may be used to verify the size of the individual
			// part
			...
			case TYPE_AUDIO_DATA:
				...
				if (backPointer != (size + 11)) {
					log.debug(
							"Data size ({}) and back pointer ({}) did not match",
							size, backPointer);
				}
				break;
		...
		// log.debug("Aggregate processing complete, {} parts extracted",
		// parts.size());
		return parts;
	}
	...
	public String toString() {
		return String.format("Aggregate - ts: %s length: %s", getTimestamp(),
