...
/**
 * Collection of commonly used constants with status codes. Descriptions
 * provided as in FMS 2.0.1 documentation available at adobe.com with some minor
 * additions and comments.
 */
public interface StatusCodes { // 状态码接口
	/**
	...
	/**
	 * The URI specified in the NetConnection.connect method did not specify
	 * 'rtmp' as the protocol. 'rtmp' must be specified when connecting to FMS
	 * and Red5. Either not supported version of AMF was used (3 when only 0 is
	 * supported)
	 */
	 ...
	 * application expected different parameters from those that were passed, or
	...
	/**
	 * The subscriber has used the seek command to move to a particular location
	...
	/**
	 * The ActionScript engine has encountered a runtime error. In addition to
	 * the standard infoObject properties, the following properties are set:
	 * 
	 * filename: name of the offending ASC file. lineno: line number where the
	 * error occurred. linebuf: source code of the offending line.
	 */
	public static final String APP_SCRIPT_ERROR = "Application.Script.Error";
	/**
	 * The ActionScript engine has encountered a runtime warning. In addition to
	 * the standard infoObject properties, the following properties are set:
	 * 
	 * filename: name of the offending ASC file. lineno: line number where the
	 * error occurred. linebuf: source code of the offending line
	...
	/**
	 * This information object is passed to the onAppStop event handler when the
	 * application instance is about to be destroyed by the server.
	 */
	...
	/**
	 * The persistence parameter passed to SharedObject.getRemote() is different
	 * from the one used when the shared object was created.
	 */
	public static final String SO_PERSISTENCE_MISMATCH = "SharedObject.BadPersistence";

	/**
	 * This event is sent if the player detects an MP4 with an invalid file
	 * structure. Flash Player cannot play files that have invalid file
	 * structures.
	 * 
	 * New for FMS3
	 */
	...
	/**
	 * This event is sent if the player does not detect any supported tracks. If
	 * there aren't any supported video, audio or data tracks found, Flash
