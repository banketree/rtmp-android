 ...
 */
public class OutboundHandshake extends RTMPHandshake { // 代理握手

	private static final byte[] SERVER_CONST = "Genuine Adobe Flash Media Server 001"
			.getBytes();

	private static final byte[] CLIENT_CONST = "Genuine Adobe Flash Player 001"
			.getBytes();
	...
	/**
	 * Generates initial handshake request, validates response, and generates
	 * final handshake.
	 * 
	 * @param input
	 *            incoming RTMP bytes
	 ...
	 */
	public IoBuffer doHandshake(IoBuffer input) { // 执行握手
	 ...
	 */
	@Override
	protected void createHandshakeBytes() { // 创建握手
		...
		System.arraycopy(rndBytes, 0, handshakeBytes, 8,
				(Constants.HANDSHAKE_SIZE - 8));
	}
	 ...
	 * @return outgoing handshake
	 */
	public IoBuffer generateClientRequest1() { // 生成请求
		...
		if (handshakeType == RTMPConnection.RTMP_ENCRYPTED) {
			...
			buf.rewind();
			int messageLength = Constants.HANDSHAKE_SIZE
					- RTMPHandshake.DIGEST_LENGTH;
			...
			buf.get(message, digestOffset, Constants.HANDSHAKE_SIZE
					- afterDigestOffset);
	...
	public boolean decodeServerResponse(IoBuffer in) { // 解码
		...
		if (handshakeType == RTMPConnection.RTMP_ENCRYPTED) {
			...
			// log.debug("Server version: {}",
			// Hex.encodeHexString(serverVersion));
			...
			if (Arrays.equals(digest, incomingDigest)) {
				...
				if (Arrays.equals(digest, incomingDigest)) {
					...
					throw new RuntimeException(
							"Type 1 digest comparison failed");
				}
			}
			// log.debug("server public key: {}",
			// Hex.encodeHexString(incomingPublicKey));
			byte[] sharedSecret = getSharedSecret(incomingPublicKey,
					keyAgreement);
			// log.debug("shared secret: {}",
			// Hex.encodeHexString(sharedSecret));

			byte[] digestOut = calculateHMAC_SHA256(incomingPublicKey,
					sharedSecret);
			try {
				cipherOut = Cipher.getInstance("RC4");
				cipherOut.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(
						digestOut, 0, 16, "RC4"));
			} catch (Exception e) {
				throw new RuntimeException(e);
			}

			byte[] digestIn = calculateHMAC_SHA256(outgoingPublicKey,
					sharedSecret);
			try {
				cipherIn = Cipher.getInstance("RC4");
				cipherIn.init(Cipher.DECRYPT_MODE, new SecretKeySpec(digestIn,
						0, 16, "RC4"));
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
		...
		partTwo.put(bytes, 1 + Constants.HANDSHAKE_SIZE,
		...
		if (handshakeType == RTMPConnection.RTMP_ENCRYPTED) {
			...
			byte[] message = new byte[Constants.HANDSHAKE_SIZE
					- RTMPHandshake.DIGEST_LENGTH];
			partTwo.get(message);
			byte[] digest = calculateHMAC_SHA256(outgoingDigest,
					RTMPHandshake.GENUINE_FMS_KEY);
		...
		/*
		 * // swf verification if (swfHash != null) { byte[] bytesFromServer =
		 * new byte[RTMPHandshake.DIGEST_LENGTH];
		 * buf.position(Constants.HANDSHAKE_SIZE - RTMPHandshake.DIGEST_LENGTH);
		 * buf.get(bytesFromServer); byte[] bytesFromServerHash =
		 * calculateHMAC_SHA256(swfHash.getBytes(), bytesFromServer); //
		 * construct SWF verification pong payload IoBuffer swfv =
		 * IoBuffer.allocate(42); swfv.put((byte) 0x01); swfv.put((byte) 0x01);
		 * swfv.putInt(session.getSwfSize()); swfv.putInt(session.getSwfSize());
		 * swfv.put(bytesFromServerHash); byte[] swfvBytes = new byte[42];
		 * swfv.flip(); swfv.get(swfvBytes);
		 * session.setSwfVerification(swfvBytes); log.info(
		 * "initialized swf verification response from swfSize = {} & swfHash = '{}': {}"
		 * , swfSize, swfHash, Hex.encodeHexString(swfvBytes)); }
		 */
		return true;
	}

	public IoBuffer generateClientRequest2() { // 生成客户端请求
		...
		if (handshakeType == RTMPConnection.RTMP_ENCRYPTED) {
			...
			byte[] digest = calculateHMAC_SHA256(incomingDigest,
					RTMPHandshake.GENUINE_FP_KEY);
			byte[] message = new byte[Constants.HANDSHAKE_SIZE
					- RTMPHandshake.DIGEST_LENGTH];
			buf.rewind();
			buf.get(message);
			...
			// both parties *pretend* as if handshake part 2 (1536 bytes) was
			// encrypted
			// effectively this hides / discards the first few bytes of
			// encrypted session
			// which is known to increase the secure-ness of RC4
			// RC4 state is just a function of number of bytes processed so far
			// that's why we just run 1536 arbitrary bytes through the keys
			// below
	...
	private int addBytes(byte[] bytes) { // 添加字节
		if (bytes.length != 4) {
			throw new RuntimeException("Unexpected byte array size: "
					+ bytes.length);
		}
		int result = 0;
	...
	private int calculateOffset(byte[] pointer, int modulus, int increment) { // 计算位移
	...
	protected byte[] getFourBytesFrom(IoBuffer buf, int offset) { // 获取数据
	 ...
	 * 
	 * @param input
	 * @return true if client used a supported validation scheme, false if
	 *         unsupported
	 */
	@Override
	public boolean validate(IoBuffer input) { // 有效
	...
	private boolean validateScheme(byte[] pBuffer, int scheme) { // 有效签名
		int digestOffset = -1;
		switch (scheme) {
		case 0:
		...
		System.arraycopy(pBuffer, digestOffset + DIGEST_LENGTH, tempBuffer,
				digestOffset, Constants.HANDSHAKE_SIZE - digestOffset
						- DIGEST_LENGTH);
		...
		for (int i = 0; i < DIGEST_LENGTH; i++) {
			// log.trace("Digest: {} Temp: {}", (pBuffer[digestOffset + i] &
			// 0x0ff), (tempHash[i] & 0x0ff));
