 ...
 */
public class RTMPEIoFilter extends IoFilterAdapter {// rtmpe IO滤波器
	...
	public void messageReceived(NextFilter nextFilter, IoSession session,
		...
		if (session.containsAttribute(RTMPConnection.RTMP_HANDSHAKE)) {
			...
			if (handshakeType == 0) {
				...
				rtmp.setEncrypted(handshakeByte == RTMPConnection.RTMP_ENCRYPTED ? true
						: false);
				...
				// log.trace("Bytes read: {} written: {}", readBytesCount,
				// writeBytesCount);
				// don't remove the handshake when using RTMPE until we've
				// written all the handshake data
				if (writeBytesCount >= (Constants.HANDSHAKE_SIZE * 2)) {
					...
					if (session
							.containsAttribute(RTMPConnection.RTMPE_CIPHER_IN)) {
						// log.debug("Ciphers already exist on the session");
					} else {
						// log.debug("Adding ciphers to the session");
						session.setAttribute(RTMPConnection.RTMPE_CIPHER_IN,
								handshake.getCipherIn());
						session.setAttribute(RTMPConnection.RTMPE_CIPHER_OUT,
								handshake.getCipherOut());
					}
				}
			}
		}
		Cipher cipher = (Cipher) session
				.getAttribute(RTMPConnection.RTMPE_CIPHER_IN);
		if (cipher != null) { // may want to verify handshake is complete as
								// well
	...
	public void filterWrite(NextFilter nextFilter, IoSession session,
		...
		Cipher cipher = (Cipher) session
				.getAttribute(RTMPConnection.RTMPE_CIPHER_OUT);
		if (cipher != null) { // may want to verify handshake is complete as
								// well
			...
			if (!message.hasRemaining()) {
				...
				nextFilter.filterWrite(session, new EncryptedWriteRequest(
						request, messageEncrypted));
			}
		} else {
